---
document_id: "QATL-ATLAS-1000-ATLAS-000-009-00-004-070-SERVICE-GSE-INTERFACES-GENERAL"
title: "ATLAS 000-009 · 00.004.070 — Service GSE Interfaces General"
register: ATLAS-1000
...
subsubject: "070"
subsubject_title: "Service GSE Interfaces General"
subsubject_file: "004-070-Service-GSE-Interfaces-General.md"
subsubject_link: "./004-070-Service-GSE-Interfaces-General.md"
...
# ATLAS 000-009 · Section 00 · Subsection 004 · 070 — Service GSE Interfaces General
...
Documents the general ground support equipment (GSE) interfaces for the programme-defined aircraft type servicing operations. Covers the HVDC ground power unit (GPU) interface (270 V DC bus), pre-conditioned air (PCA) cart interface, refuelling bowser interface, water and waste servicing vehicle interfaces, and GSE compatibility requirements.
...
| S1000D SNS | 004-070-00 | ![To Be Completed](https://img.shields.io/badge/To_Be_Completed-orange) |
...
The **Service GSE Interfaces General** node defines the general interface requirements between the programme-defined aircraft type and the Ground Support Equipment (GSE) fleet. The dominant new GSE type for the [PROGRAMME-VARIANT] is the HVDC Ground Power Unit (GPU): it must supply 270 V DC ± 5% at up to 400 A (108 kW) for line charging, with a provision for 540 V DC boost charging for fast-turn operations. The GPU connector follows the proposed ACS (Aircraft Charging Standard) interface or the interim agreed [PROGRAMME-AIRCRAFT] proprietary receptacle defined in the applicable GSE Interface Control Document (ICD).

Towing and pushback GSE interfaces are conventional (towbar or towbarless tractor at nose gear), but the tractor must respect the [PROGRAMME-VARIANT] nose-gear steering-angle limit and must not activate during HVDC connection. Passenger boarding bridges, cargo loaders, and catering vehicles are electrically neutral — they interface with the aircraft via standard ATA 416 and IATA AHM standards. All GSE must be bonded to the aircraft ground point before fluid or electrical connection, per the [PROGRAMME-VARIANT] Ground Handling Manual (GHM) electrostatic bonding requirements.
...
- controlled definition of the service gse interfaces general scope;
...
The **Service GSE Interfaces General** architecture is organized around controlled interfaces, deterministic function allocation, and maintainable component boundaries within the 000-009 General Information and Service section of the programme-defined aircraft type programme.
...
|---:|---|---|---|---|
| F-001 | Core Function | Primary controlled function for service gse interfaces general. | [Interfaces](#10-interfaces) | ![TBD](https://img.shields.io/badge/TBD-red) |
...
```mermaid
flowchart LR
    A[Aircraft-Level Function] --> B[Service GSE Interfaces General]
...
*Diagram 1 — System context for Service GSE Interfaces General.*
...
```mermaid
flowchart TB
    SYS[Service GSE Interfaces General] --> F1[Core Function]
...
*Diagram 2 — Internal functional architecture for Service GSE Interfaces General.*
...
| DMC prefix | `DMC-<PROGRAMME>-<VARIANT>-004-070` | ![TBD](https://img.shields.io/badge/TBD-red) |
...
|---|---|---|---|
| OI-004-070-001 | Content requires review by system expert. | Q-GROUND | open |
| OI-004-070-002 | S1000D DMC mapping to be validated against DMRL. | Q-DATAGOV | open |
