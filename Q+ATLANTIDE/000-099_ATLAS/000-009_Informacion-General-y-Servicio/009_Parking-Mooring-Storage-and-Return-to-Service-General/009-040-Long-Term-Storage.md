...
Documents the long-term storage procedures (30 days to 24 months) for the programme-defined aircraft type. Covers the controlled aircraft preservation configuration, HVDC battery hibernation mode, engine long-term preservation (corrosion inhibitor, desiccant), hydraulic system (TBD) preservation, and the full preservation/depreservation task list with intervals.
...
The **Long-Term Storage** node defines the procedures, preservation requirements, and periodic inspection schedule for parking the programme-defined aircraft type out of service for more than 30 days, up to a maximum of 24 months. Long-term storage (LTS) imposes more extensive preservation actions than short-term storage, particularly for: (A) traction battery pack — SoC target 50%; HVDC GPU trickle-charge maintained; battery management system (BMS) performs a full cell-balance cycle every 30 days; monthly inspection includes visual check of battery enclosure vents and thermal management coolant level; (B) electric drive systems — ENWTD and ENWS actuator lubricant preservation per AMM 09-40 (manual shaft rotation every 60 days to prevent bearing false-brinelling); (C) fuel system — tanks drained and purged per AMM 28-40 ([PROGRAMME-VARIANT] fuel is Jet A-1/SAF blend; microbial growth inhibitor STADIS-450 added at 1:1 000 ratio before drain); (D) hydraulic system — not applicable (no hydraulic system on the [PROGRAMME-VARIANT]); (E) engine — each CFM RISE-derived turbofan preserved per CMM 72-00 (fog-oil spray, all ports sealed with moisture-absorbing desiccant plugs).

Monthly LTS inspection checklist (Q-PSMM-LTS-001): 78-step checklist, includes: tyre pressure check (nitrogen fill to 220 psi; check for flat-spot if on jacks), landing gear door seal inspection, PSMM data download, antenna cover condition, corrosion spot-check per AMM 57-10. At 12 months, an extended LTS inspection adds NDT of nacelle composite cowl panels and torque-check of engine mount bolts. Maximum LTS duration without engineering review: 24 months. Extension beyond 24 months requires Q-ENG-LTS-REVIEW process (issued by Q+ATLANTIDE Airworthiness group).
...
The **Long-Term Storage** architecture is organized around controlled interfaces, deterministic function allocation, and maintainable component boundaries within the 000-009 General Information and Service section of the programme-defined aircraft type programme.
...
| DMC prefix | `DMC-<PROGRAMME>-<VARIANT>-009-040` | ![TBD](https://img.shields.io/badge/TBD-red) |
