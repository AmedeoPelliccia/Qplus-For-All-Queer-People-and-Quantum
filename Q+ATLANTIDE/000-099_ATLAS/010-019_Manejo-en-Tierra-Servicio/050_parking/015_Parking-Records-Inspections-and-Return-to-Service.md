---
document_id: QATL-ATLAS-1000-ATLAS-010-019-01-050-015-PARKING-RECORDS-INSPECTIONS-AND-RETURN-TO-SERVICE
title: "ATLAS 010-019 · 01.050.015 — Parking Records, Inspections and Return to Service"
register: ATLAS-1000
...
- **The `event_classification:` field — the propagation key.** Every parking record-type definition declares a top-level YAML field `event_classification:` whose value is one of:
  - `nominal` — all parameters within steady-state limits; no environmental event reached the L1+ action level of [`./03` §0](./013_Mooring-Tie-Down-and-Wind-Protection.md#0-invariants-machine-checkable); walk-around clean; no FOD found. The record is filed; **no maintenance action** is propagated.
...
- **Record types covered.**
  - **Parking event log** — start time, end time, parking class per [`./01`](./011_Scope-and-Parking-Boundaries.md) (turnaround / overnight / extended), stand identifier and stand class per [`./02`](./012_Parking-Configurations-and-Stand-Types.md), configuration snapshot per [`./04`](./014_Short-Term-Parking-and-Turnaround-Configurations.md) (chocks in/out timestamps, parking-brake state, gear-pin in/out timestamps, APU/GPU source and switch-over timestamps, door states, ACU hookup timestamps), wind action level reached per [`./03`](./013_Mooring-Tie-Down-and-Wind-Protection.md), and the resulting `event_classification:`.
  ...
  - **Return-to-service (RTS) clearance** — the formal record that closes the parking event. Required: walk-around clean (or all anomalies cleared), all configuration cross-checks passed (gear pins out, probe covers off, control locks released, doors in expected state, parking brake released only after chocks removed), `event_classification:` recorded. Without an RTS record the aircraft is not considered released; pushback per [`../040_remolque/`](../040_remolque/010_Overview.md) shall not commence.
- **Bidirectional link with `LC11_MAINTENANCE/`.** The propagation is bidirectional: parking records consumed by LC11 can in turn raise *additional* inspection items (e.g. tightened wind-event inspection scope after a fleet-wide finding), which flow back as updated content for [`./03`](./013_Mooring-Tie-Down-and-Wind-Protection.md) and [`./04`](./014_Short-Term-Parking-and-Turnaround-Configurations.md). This subsection is the canonical *producer* of parking events; LC11 is the canonical consumer for maintenance triggers.
- **Out of scope.** The wind-action decision matrix itself (subsubject `013`), the physical-configuration definitions (subsubject `014`), the stand classification (subsubject `012`), the parking-class duration thresholds (subsubject `011`), and the maintenance task content beyond the trigger interface (`AMPEL360-AIR-T/LC11_MAINTENANCE/` SSOT).
...
| Subsubject | `015` — Parking Records, Inspections and Return to Service |
| Primary Q-Division | Q-GROUND[^qdiv] |
| Support Q-Divisions | Q-MECHANICS, Q-INDUSTRY |
| ORB support | ORB-PMO, ORB-FIN |
| Governance class | `baseline`[^gov] |
| Folder path | `Q+ATLANTIDE/000-099_ATLAS/010-019_Manejo-en-Tierra-Servicio/050_parking/` |
| Document | `015_Parking-Records-Inspections-and-Return-to-Service.md` (this file) |
