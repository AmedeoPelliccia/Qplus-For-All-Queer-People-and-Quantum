---
document_id: QATL-ATLAS-1000-ATLAS-010-019-01-060-012-GSE-CATALOG-AND-COMPATIBILITY-MATRIX
title: "ATLAS 010-019 · 01.060.012 — GSE Catalog and Compatibility Matrix"
register: ATLAS-1000
...
language: en
---
# ATLAS 010-019 · Section 01 · Subsection 060 · Subsubject 012 — GSE Catalog and Compatibility Matrix
...
This subsubject defines **what the population is and how compatible it is**. The corresponding **powered/non-powered classification and energy-source roadmap** for catalog entries is owned by [`./013_Powered-and-Non-Powered-GSE.md`](./013_Powered-and-Non-Powered-GSE.md). The **coupling/interface specification** for each catalog entry is owned by [`./014_GSE-Interfaces-Couplings-and-Aircraft-Side-Connections.md`](./014_GSE-Interfaces-Couplings-and-Aircraft-Side-Connections.md). The **lifecycle, calibration and records** for each catalog entry are owned by [`./015_GSE-Maintenance-Calibration-and-Records.md`](./015_GSE-Maintenance-Calibration-and-Records.md).

## 2. Scope

- Covers the *GSE Catalog and Compatibility Matrix* subsubject (`012`) of subsection `060` *GSE* within section `01` *Manejo en Tierra & Servicio*.
...
- The catalog covers the GSE classes relevant to **operational** support. Maintenance-bay-only equipment (jacks, hangar cranes, alignment fixtures) is owned by the operator's tooling program per the boundary in [`./01`](./011_Scope-and-GSE-Boundaries.md).

### 2.2 Compatibility matrix — AMPEL360 family × GSE classes

The compatibility matrix carries **at minimum three columns** (per the Overview's mandate restated in [`./010_Overview.md` §2](./010_Overview.md#2-scope)):
...
| GSE-EP-* | GPU | `compatible` | `compatible-with-mod` | Standard 115 V AC 400 Hz GPU; BWB-Q100 receptacle keying differs (AMPEL360-specific anti-misconnect key) — connector-side adapter required. Software interlock per [`./04`](./014_GSE-Interfaces-Couplings-and-Aircraft-Side-Connections.md). |
...
| GSE-FH-* | LH₂ fuel truck / dispenser | n/a | `incompatible-A360` | **No ICAO baseline.** AMPEL360-specific LH₂ dispenser required, with H₂ coupling per [`./04`](./014_GSE-Interfaces-Couplings-and-Aircraft-Side-Connections.md) and certification overlay from `ATA_IN_H2_GSE_AND_SUPPLY_CHAIN/`[^h2ns]. |
...
5. **AMPEL360 connector keying** — `GSE-EP-*` requires AMPEL360-specific anti-misconnect keying on the BWB-Q100 (see [`./04`](./014_GSE-Interfaces-Couplings-and-Aircraft-Side-Connections.md)).
...
- Out of scope for this subsubject: the procedural use of these units (already covered by `010`–`050`); the powered/non-powered taxonomy (owned by `013_`); the per-row coupling specification (owned by `014_`); the lifecycle and records (owned by `015_`).
...
| Subsection | `060` — GSE |
| Subsubject | `012` — GSE Catalog and Compatibility Matrix |
| Primary Q-Division | Q-GROUND[^qdiv] |
| Support Q-Divisions | Q-MECHANICS, Q-INDUSTRY |
| ORB support | ORB-PMO, ORB-FIN |
| Governance class | `baseline`[^gov] |
| Folder path | `Q+ATLANTIDE/000-099_ATLAS/010-019_Manejo-en-Tierra-Servicio/060_GSE/` |
| Document | `012_GSE-Catalog-and-Compatibility-Matrix.md` (this file) |
