...
| 011 | Scope and GSE Boundaries | [`011_Scope-and-GSE-Boundaries.md`](./011_Scope-and-GSE-Boundaries.md) | active |
| 012 | GSE Catalog and Compatibility Matrix | [`012_GSE-Catalog-and-Compatibility-Matrix.md`](./012_GSE-Catalog-and-Compatibility-Matrix.md) | active |
| 013 | Powered and Non-Powered GSE | [`013_Powered-and-Non-Powered-GSE.md`](./013_Powered-and-Non-Powered-GSE.md) | active |
| 014 | GSE Interfaces, Couplings and Aircraft-Side Connections | [`014_GSE-Interfaces-Couplings-and-Aircraft-Side-Connections.md`](./014_GSE-Interfaces-Couplings-and-Aircraft-Side-Connections.md) | active |
| 015 | GSE Maintenance, Calibration and Records | [`015_GSE-Maintenance-Calibration-and-Records.md`](./015_GSE-Maintenance-Calibration-and-Records.md) | active |
...
Governed by [`organization/Q+ATLANTIDE.md`](../../../../organization/Q+ATLANTIDE.md)[^baseline]. All subsubjects under this subsection inherit `architecture_code = ATLAS`, `primary_q_division = Q-GROUND` and `governance_class = baseline` from the parent ATLAS band; extensions added under `016`–`019` shall preserve those header fields, follow the canonical `01N_*.md` naming scheme, and reuse the footnote set declared below. The **inversion rule** against `010`–`050` (GSE-as-subject vs. GSE-as-supporting-context) stated in [`./010_Overview.md`](./010_Overview.md) and restated above is the structurally hardest boundary in the range and shall not be weakened by future edits; contributors shall apply the inversion test before adding GSE content to either side. Subsubject `012` carries a **three-column** compatibility matrix (`compatible` / `compatible-with-mod` / `incompatible-A360`); the third column is mandatory and shall not be collapsed, because it is what makes the matrix operationally useful for station qualification. Subsubject `015` declares the records contract `record_class: GSE_evidence` and the contractual hook for non-operator-owned units; both shall be treated as binding clauses on the GSE evidence chain into AEROSPACEMODEL / DPP / SSOT, not as advisory text. H₂-specific entries in `012_` / `013_` / `014_` / `015_` declare into `OPT-INS_FRAMEWORK/I-INFRASTRUCTURES/ATA_IN_H2_GSE_AND_SUPPLY_CHAIN/`[^h2ns] for the certification overlay; the catalog and coupling specifications are authored here, the H₂-specific compliance evidence is overlaid there, and that direction shall be preserved.

## 7. Change Log

| Version | Date | Author | Change |
|---|---|---|---|
| 1.0.0 | 2026-05-07 | Q-GROUND | Initial population of subsection `060 GSE`: README + Overview enrichment (inversion-rule opening line, diagram, ATA 12 / ATA 09 cross-refs, `ATA_IN_H2_GSE_AND_SUPPLY_CHAIN/` adjacency, numbering note disambiguating against ATA 60, triangulation against `010`/`020`/`030`/`040`/`050`) + subsubjects `011`–`015`, including the four-pattern ownership model and airside/landside split in `011_`, the enumerated catalog with three-column compatibility matrix and the H₂ deviation list in `012_`, the powered/non-powered split with the diesel → electric → H₂ transition roadmap (BWB-Q100 station-qualification gate) in `013_`, the aircraft-side × GSE-side coupling table with the AMPEL360-specific H₂ couplings and certification regime in `014_`, and the records contract mandating `record_class: GSE_evidence` in the AEROSPACEMODEL / DPP / SSOT evidence chain with the contractual hook for non-operator-owned units in `015_`. |
