---
document_id: QATL-ATLAS-1000-ATLAS-010-019-01-060-013-POWERED-AND-NON-POWERED-GSE
title: "ATLAS 010-019 · 01.060.013 — Powered and Non-Powered GSE"
register: ATLAS-1000
...
| GSE-EP-* (GPU) | Diesel-genset | **Electric (mains-fed) / Battery** | BWB-Q100 receptacle keying must be honoured (see [`./04`](./014_GSE-Interfaces-Couplings-and-Aircraft-Side-Connections.md)). |
| GSE-AS-* (ASU) | Diesel-driven compressor | **Electric compressor / mains-fed** | None. |
| GSE-AC-* (ACU / PCA) | Diesel-driven chiller | **Electric / mains-fed** | BWB-Q100 cabin volume requires higher throughput (see `012_`). |
...
| GSE-DI-* (de-ice truck) | Diesel chassis + heated fluid | **Electric chassis + heated fluid (mains-warmed)** | BWB-Q100 wing area requires extended boom (see `012_`); no energy-source blocker. |
...
- Out of scope: the per-row coupling specification (`014_`); the per-row maintenance/calibration regime (`015_`); the procurement contract templates (owned by ORB-FIN); the airport-decarbonisation regulatory framework as such.
