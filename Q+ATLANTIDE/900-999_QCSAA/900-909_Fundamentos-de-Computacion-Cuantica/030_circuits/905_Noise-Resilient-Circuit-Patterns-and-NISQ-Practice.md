...
- Concepts in scope:
  ...
  - **Noise-aware circuit design.** Choose ansatz topology to match the device's connectivity graph (avoid SWAPs); place high-fidelity gates on the critical path; place idle qubits on the highest-$T_2$ wires; align measurement to the end of the longest-$T_1$ qubits. These are circuit-design rules of thumb that *consume* the per-qubit calibration data of [`../010_Qubits/04_`](../010_Qubits/04_Decoherence-Noise-and-Fidelity.md) and [`../901_gates/005_`](../901_gates/005_Gate-Implementation-Calibration-and-Error-Characterization.md).
