...
- Out of scope: the gate catalogue (`../020_gates/`), the abstract circuit (`901_`), the depth/width metrics being budgeted (`902_`), the dynamic-circuit primitives consumed by these patterns (`903_`), the compilation pipeline that emits the executable patterns (`904_`), and active quantum error correction (`../010_Qubits/05_`).
