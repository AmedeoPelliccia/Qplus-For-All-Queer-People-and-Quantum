...
| Subsubject namespace | `000`–`099` (`000` + `001`–`005` populated; `NNN_*.md` scheme reset within subsection) |
...
Governed by [`organization/Q+ATLANTIDE.md`](../../../../organization/Q+ATLANTIDE.md)[^baseline]. All subsubjects under this subsection inherit `architecture_code = QCSAA`, `primary_q_division = Q-HORIZON` and `governance_class = restricted` from the parent QCSAA band; extensions added under `006`–`099` shall preserve those header fields, follow the `NNN_*.md` naming scheme reset within this subsection, and reuse the footnote set declared below.
...
|---|---|---|---|
| 1.0.0 | 2026-05-07 | Q-HORIZON | Initial baseline: `000_Overview.md` plus subsubjects `001`–`005`, using the `NNN_*.md` subsubject scheme reset within the `900_Qubits/` subsection. Subsection index established. |
