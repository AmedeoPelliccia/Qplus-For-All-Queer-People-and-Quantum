---
document_id: QATL-ATLAS-1000-QCSAA-900-909-00-901-001-GATE-DEFINITION-AND-UNITARY-FORMALISM
...
Defines the **quantum gate** as a unitary operator on the Hilbert space of one or more qubits, and establishes the formalism — unitarity, reversibility, matrix representation, composition, and the Hamiltonian origin of every gate via Schrödinger time evolution — on which all downstream chapters of `901_gates/` depend. Aligns the register with the IEEE P7130 vocabulary[^ieeep7130] and with the controlled Q+ATLANTIDE baseline[^baseline].
...
- Concepts in scope:
  - **Gate as unitary operator.** A quantum gate on $n$ qubits is a unitary operator $U: \mathcal{H}_n \to \mathcal{H}_n$ acting on the $2^n$-dimensional Hilbert space defined in [`../900_Qubits/001_Qubit-Definition-and-Mathematical-Formalism.md`](../900_Qubits/001_Qubit-Definition-and-Mathematical-Formalism.md).
  ...
  - **Reversibility.** Every quantum gate has an inverse $U^{-1} = U^\dagger$. There is **no quantum analogue of irreversible classical gates** (AND, OR, NAND in their standard two-input/one-output forms): irreversible logic must be embedded into reversible logic (Toffoli/Fredkin, see `003_`) before it can be executed quantumly.
  ...
  - **Hamiltonian origin (Schrödinger evolution).** Every gate is the time-evolution operator of some controlled Hamiltonian: $U = e^{-iHt/\hbar}$. This is the bridge between the abstract unitary algebra of `002_`–`004_` and the **physical pulse-level realization** in `005_`.
  - **Why unitarity is forced by the postulates.** Closed-system quantum mechanics (Schrödinger picture) requires linear, norm-preserving evolution; norm preservation plus linearity forces unitarity. Non-unitary maps are admissible only in the **open-system** picture — i.e. they describe interaction with an environment, which in `901_gates/` is treated as **noise** (see [`../900_Qubits/004_Decoherence-Noise-and-Fidelity.md`](../900_Qubits/004_Decoherence-Noise-and-Fidelity.md) and `005_` of this chapter), not as a primitive operation.
- Out of scope: the catalogue of single-qubit gates (`002_`), multi-qubit / entangling gates (`003_`), universal sets and decomposition (`004_`), and physical pulse-level realization, calibration, and benchmarking (`005_`).

## 3. Diagram — From Hamiltonian to Composed Unitary

The pipeline below shows the **single causal chain** from a controlled physical Hamiltonian to an executed quantum gate, and from individual gates to composed operations. The right-hand branch shows the constraint that closes the loop: every link in the chain must preserve the unitarity condition $U^\dagger U = I$, otherwise the operation falls outside the scope of `901_gates/` and is reclassified as noise.
...
flowchart TD
    ...
    C -. "violated ⇒" .-> N["Open-system map<br/>= noise (900_/004_, 05_)"]
```
