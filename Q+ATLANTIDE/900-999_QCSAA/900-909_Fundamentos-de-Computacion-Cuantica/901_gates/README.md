...
- Covers the full subsubject namespace `000`–`009` of subsection `901` *gates* under the canonical Q+ATLANTIDE Subsubject scheme `00N_*.md` (per §2.2 of the QCSAA README); subsubjects `001`–`005` are populated in this baseline release, the remaining `006`–`009` slots remain available for future extension per the Overview's authorisation[^archtable].
...
- **Upstream (within `900-909`)** — [`../010_Qubits/`](../010_Qubits/). Every gate in this chapter acts on the Hilbert space defined in `010_Qubits/901_`; every per-modality fidelity and gate-time number in `005_` is bounded by the $T_1, T_2$ characterisation in `010_Qubits/904_`; every reference to overhead in `004_` compounds with the physical-to-logical ratio in `010_Qubits/905_`.
- **Downstream (within `900-909`)** — [`../030_circuits/`](../030_circuits/), [`../040_quantum-algorithms/`](../040_quantum-algorithms/), [`../050_foundations/`](../050_foundations/). The chapter boundary against `030_circuits/` is stated explicitly in `000_Overview.md` §2 and `004_` §3.
...
|---|---|---|---|
| 1.0.0 | 2026-05-07 | Q-HORIZON | Initial baseline: `000_Overview.md` plus subsubjects `001`–`005`, using the canonical Q+ATLANTIDE Subsubject scheme `00N_*.md` (per §2.2 of the QCSAA README); chapter parallel to `010_Qubits/` (definition → categories → composition → universality → physical realization). |
