...
- Concepts in scope:
  ...
  - **Random circuit sampling (RCS)** — a quantum computer applies a random depth-d circuit of 2-qubit gates to n qubits and samples the output distribution. Google Sycamore (2019) used 53 qubits at depth ~20, claiming a runtime that would require 10,000 years on a classical supercomputer. The hardness argument (Aaronson & Chen[^aaronson]) combines anti-concentration of the output distribution with a polynomial-hierarchy (PH) collapse argument: efficiently spoofing the heavy-output set probability in the worst case would collapse PH to the third level.
