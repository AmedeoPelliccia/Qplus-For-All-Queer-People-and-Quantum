---
document_id: QATL-ATLAS-1000-QCSAA-900-909-00-020-01-GATE-DEFINITION-AND-UNITARY-FORMALISM
title: "QCSAA 900-909 · 00.020.01 — Gate Definition and Unitary Formalism"
register: ATLAS-1000
...
language: en
---
# QCSAA 900-909 · Section 00 · Subsection 020 · Subsubject 01 — Gate Definition and Unitary Formalism
...
- Concepts in scope:
  - **Gate as unitary operator.** A quantum gate on $n$ qubits is a unitary operator $U: \mathcal{H}_n \to \mathcal{H}_n$ acting on the $2^n$-dimensional Hilbert space defined in [`../010_Qubits/01_Qubit-Definition-and-Mathematical-Formalism.md`](../010_Qubits/01_Qubit-Definition-and-Mathematical-Formalism.md).
  ...
  - **Reversibility.** Every quantum gate has an inverse $U^{-1} = U^\dagger$. There is **no quantum analogue of irreversible classical gates** (AND, OR, NAND in their standard two-input/one-output forms): irreversible logic must be embedded into reversible logic (Toffoli/Fredkin, see `03_`) before it can be executed quantumly.
  ...
  - **Hamiltonian origin (Schrödinger evolution).** Every gate is the time-evolution operator of some controlled Hamiltonian: $U = e^{-iHt/\hbar}$. This is the bridge between the abstract unitary algebra of `02_`–`04_` and the **physical pulse-level realization** in `05_`.
  - **Why unitarity is forced by the postulates.** Closed-system quantum mechanics (Schrödinger picture) requires linear, norm-preserving evolution; norm preservation plus linearity forces unitarity. Non-unitary maps are admissible only in the **open-system** picture — i.e. they describe interaction with an environment, which in `020_gates/` is treated as **noise** (see [`../010_Qubits/04_Decoherence-Noise-and-Fidelity.md`](../010_Qubits/04_Decoherence-Noise-and-Fidelity.md) and `05_` of this chapter), not as a primitive operation.
- Out of scope: the catalogue of single-qubit gates (`02_`), multi-qubit / entangling gates (`03_`), universal sets and decomposition (`04_`), and physical pulse-level realization, calibration, and benchmarking (`05_`).
...
flowchart TD
    ...
    C -. "violated ⇒" .-> N["Open-system map<br/>= noise (010_/04_, 05_)"]
```
