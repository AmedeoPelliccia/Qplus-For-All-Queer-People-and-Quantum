---
document_id: QATL-ATLAS-1000-STA-100-109-00-106-000-GENERAL
title: "STA 100-109 · 106-000 — General"
register: ATLAS-1000
...
# STA 100-109 · 106-000 — General

## 1. Purpose

Overview entry-point for the *Salud Tripulación y Factores Humanos* subsection within STA `100-109`, introducing the crew health and human factors framework for Q+ATLANTIDE crewed missions[^baseline].

Crew health is a mission-enabling function: physiological degradation from microgravity (bone density loss ~1 % per month, muscle atrophy, fluid shift, cardiovascular deconditioning), radiation exposure (cumulative dose limits per NCRP-132[^ncrp132]), circadian disruption, and psychological stress represent the primary human health risks for long-duration space missions. The subsection establishes the integrated crew health management system — medical monitoring, countermeasures, nutrition, human factors design, psychological support, and medical data traceability.

## 2. Scope

- Covers the *General* subsubject (`000`) of subsection `106`.
...
```mermaid
flowchart TB
    HEALTH["Crew Health Management"]
    HEALTH --> MED["Medical Monitoring<br/>(vital signs · biomarkers)"]
    HEALTH --> RAD["Radiation Protection<br/>(dosimetry · ALARA)"]
    HEALTH --> CM["Countermeasures<br/>(exercise · nutrition)"]
    HEALTH --> HF["Human Factors<br/>(ergonomics · cognitive load)"]
    HEALTH --> PSY["Psychological Support<br/>(wellbeing · autonomy)"]
    HEALTH --> DATA["Medical Data Management<br/>(CSDB traceability)"]
    style HEALTH fill:#2c82c9,color:#fff
```
...
| Document | `106-000-General.md` (this file) |
