---
document_id: QATL-ATLAS-1000-STA-100-109-00-106-020-MEDICAL-MONITORING-AND-CREW-STATUS-ASSESSMENT
title: "STA 100-109 · 106-020 — Medical-Monitoring-and-Crew-Status-Assessment"
register: ATLAS-1000
...
subsubject: "020"
subsubject_title: "Medical-Monitoring-and-Crew-Status-Assessment"
...
# STA 100-109 · 106-020 — Medical-Monitoring-and-Crew-Status-Assessment

## 1. Purpose

Defines the **medical monitoring architecture and crew status assessment** protocols for Q+ATLANTIDE missions, specifying the on-board health monitoring system, telemedicine capabilities, and autonomous medical decision support per NASA-STD-3001[^nastd3001].

The on-board medical monitoring system measures: ECG (12-lead, continuous capability), blood pressure (non-invasive cuff and continuous beat-to-beat), SpO₂ (pulse oximetry), core body temperature, respiratory rate, and activity monitoring (actigraphy). For long-duration missions (> 30 days), periodic laboratory tests include: complete blood count, comprehensive metabolic panel, urinalysis, and bone mineral density (DXA-equivalent ultrasound). The crew medical officer (CMO) performs assessments with ground support via telemedicine; the autonomous medical decision support system (AMDSS) provides real-time guidance during communication blackouts.

## 2. Scope

- Covers the *Medical-Monitoring-and-Crew-Status-Assessment* subsubject (`020`) of subsection `106`.
...
## 3. Diagram — Medical-Monitoring-and-Crew-Status-Assessment

```mermaid
flowchart TB
    BIOSENS["Wearable Biosensors<br/>(ECG · SpO₂ · BP · Temp)"]
    LAB["Periodic Lab Tests<br/>(CBC · CMP · bone density)"]
    BIOSENS & LAB --> HEALTH_DB["On-board Health Database<br/>(encrypted · privacy-protected)"]
    HEALTH_DB --> CMO["Crew Medical Officer<br/>(CMO assessment)"]
    HEALTH_DB --> AMDSS["Autonomous Medical DSS<br/>(guidance during comms blackout)"]
    HEALTH_DB --> TELE["Telemedicine Downlink<br/>(ground flight surgeon)"]
    style HEALTH_DB fill:#2c82c9,color:#fff
```
...
| Subsubject | `020` — Medical-Monitoring-and-Crew-Status-Assessment |
...
| Document | `106-020-Medical-Monitoring-and-Crew-Status-Assessment.md` (this file) |
